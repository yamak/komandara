...
#(
    ...
    logic        w_debug_mode;
    logic        w_debug_taken;
    logic [31:0] w_debug_target;
    logic        w_debug_exc_taken;
    logic [31:0] w_debug_exc_target;
    ...
    assign w_pc_set    = w_ex_branch_taken || w_trap_taken || w_mret_taken || w_dret_taken ||
                         w_debug_taken || w_debug_exc_taken;
    assign w_pc_target = w_debug_taken ? w_debug_target :
                         w_debug_exc_taken ? w_debug_exc_target :
                         w_trap_taken ? w_trap_target :
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            r_id_ex <= ID_EX_NOP;
        end else if (w_flush_id || w_flush_ex) begin
            r_id_ex <= ID_EX_NOP;
            ...
            if (w_stall_id) begin
                ...
                // Once flush works correctly, valid should not be masked by w_flush_id here since the outer if handles it.
                r_id_ex.valid    <= r_if_id.valid;
            end
        end
    end
    ...
    k10_execute u_execute (
        ...
        .o_branch_taken  (w_ex_branch_taken_eval),
        .o_branch_target (w_ex_branch_target),
        .o_pc_plus       (w_ex_pc_plus)
    );

    // Branch condition strictly masked by stall phase
    assign w_ex_branch_taken = r_id_ex.valid && !w_stall_ex && w_ex_branch_taken_eval;
    ...
    always_comb begin
        if (r_id_ex.valid)       w_async_epc = r_id_ex.pc;
        else if (r_if_id.valid)  w_async_epc = r_if_id.pc;
        else                     w_async_epc = w_if_pc;
    end

    // Exception aggregation
    logic        w_exc_valid;
    logic        w_exc_from_mem;
    ...
    // =======================================================================
    //  Hardware Trigger Match Evaluation (Sdtrig)
    // =======================================================================
    // The trigger match is evaluated strictly against pipeline REGISTERS
    // (r_id_ex for execution, r_ex_mem for data watchpoints).
    // This avoids combinational loops and ensures the trapped PC (dpc)
    // exactly matches the instruction that triggered the breakpoint.
    mcontrol_t [NUM_TRIGGERS-1:0]  w_trigger_mcontrol;
    logic      [NUM_TRIGGERS-1:0][31:0] w_trigger_tdata2;

    logic      w_trigger_match;       // combinational — fed directly to CSR

    always_comb begin
        w_trigger_match = 1'b0;

        for (int i = 0; i < NUM_TRIGGERS; i++) begin
            logic enabled;
            enabled = 1'b0;

            if (w_trigger_mcontrol[i].type_id == TRIG_TYPE_MCONTROL) begin
                enabled = (w_csr_priv == PRIV_M && w_trigger_mcontrol[i].m) ||
                          (w_csr_priv == PRIV_U && w_trigger_mcontrol[i].u);

                if (enabled) begin
                    // Instruction execution breakpoint — compare against EX stage PC
                    if (w_trigger_mcontrol[i].execute && r_id_ex.valid) begin
                        if (r_id_ex.pc == w_trigger_tdata2[i]) begin
                            w_trigger_match = 1'b1;
                        end
                    end
                    // Data watchpoint — compare against MEM-stage address
                    if (r_ex_mem.valid) begin
                        if (w_trigger_mcontrol[i].store && r_ex_mem.ctrl.mem_write) begin
                            if (r_ex_mem.alu_result == w_trigger_tdata2[i]) begin
                                w_trigger_match = 1'b1;
                            end
                        end
                        if (w_trigger_mcontrol[i].load && r_ex_mem.ctrl.mem_read) begin
                            if (r_ex_mem.alu_result == w_trigger_tdata2[i]) begin
                                w_trigger_match = 1'b1;
                            end
                        end
    ...
    k10_csr #(
        ...
        .DEBUG_EXCEPTION_ADDR (DEBUG_EXCEPTION_ADDR)
    ) u_csr (
        .i_clk           (i_clk),
        .i_rst_n         (i_rst_n),
        ...
        .i_timer_irq     (i_timer_irq && r_id_ex.valid && !w_stall_ex),
        .i_sw_irq        (i_sw_irq && r_id_ex.valid && !w_stall_ex),
        .i_irq_fast      (i_irq_fast & {15{r_id_ex.valid && !w_stall_ex}}),
        .i_id_ex_valid   (r_id_ex.valid && !w_stall_ex),
        .i_debug_req     (i_debug_req),
        .i_trigger_match (w_trigger_match),
        .o_debug_mode    (w_debug_mode),
        .o_debug_taken   (w_debug_taken),
        .o_debug_target  (w_debug_target),
        .o_debug_exc_taken  (w_debug_exc_taken),
        .o_debug_exc_target (w_debug_exc_target),
        .o_step_stall    (w_step_stall),
        
        // ---- Hardware Triggers ----
        .o_trigger_mcontrol (w_trigger_mcontrol),
        .o_trigger_tdata2   (w_trigger_tdata2),
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            r_ex_mem <= EX_MEM_NOP;
        end else if (w_flush_ex || w_flush_mem) begin
            r_ex_mem <= EX_MEM_NOP;
    ...
    k10_hazard_unit u_hazard (
        ...
        .i_debug_taken   (w_debug_taken || w_debug_exc_taken),
        ...
        // Step stall
        .i_step_stall    (w_step_stall),

        // Memory origin flag
        .i_trap_is_mem   (w_exc_from_mem),
