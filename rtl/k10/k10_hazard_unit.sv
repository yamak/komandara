...
(
    ...
    input  logic        i_debug_taken,      // debug ROM entry/exception redirect
    ...
    input  logic        i_trap_is_mem,      // Trap originated from MEM stage
    ...
    input  logic        i_step_stall,       // Single-step stall
    ...
    // ID-stage stall (load-use or step stall serialization)
    logic w_id_stall;
    assign w_id_stall = w_load_use || (i_step_stall && (i_ex_valid || i_mem_valid || i_wb_valid));

    // IF-stage stall (fetch busy or ID is stalled)
    logic w_if_stall;
    ...
    always_comb begin
        ...
        if (i_debug_taken || i_trap_taken || i_mret_taken || i_dret_taken) begin
            // Full pipeline flush
            o_flush_if  = 1'b1;
            o_flush_id  = 1'b1;
            o_flush_ex  = 1'b1;
            o_flush_mem = i_trap_taken ? i_trap_is_mem : 1'b0;
