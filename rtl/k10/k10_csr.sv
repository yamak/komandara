...
#(
    ...
    parameter logic [31:0] DEBUG_HALT_ADDR      = 32'h4000_3800,
    parameter logic [31:0] DEBUG_EXCEPTION_ADDR = 32'h4000_3810
)(
    input  logic        i_clk,
    input  logic        i_rst_n,
    ...
    input  logic [31:0] i_async_epc,         // Oldest valid PC for async traps
    ...
    input  logic        i_id_ex_valid,       // Valid instruction in EX stage

    // ---- Debug ----
    input  logic        i_debug_req,         // External debug halt request
    input  logic        i_trigger_match,     // Hardware trigger match
    output logic        o_debug_mode,        // Currently in debug mode
    output logic        o_debug_taken,
    output logic [31:0] o_debug_target,
    output logic        o_debug_exc_taken,
    output logic [31:0] o_debug_exc_target,
    output logic        o_step_stall,

    // ---- Hardware Triggers ----
    output mcontrol_t [NUM_TRIGGERS-1:0]  o_trigger_mcontrol,
    output logic      [NUM_TRIGGERS-1:0][31:0] o_trigger_tdata2,
    ...
    // --- Hardware Triggers (Sdtrig) ---
    logic [31:0] r_tselect; // We only support up to NUM_TRIGGERS
    mcontrol_t [NUM_TRIGGERS-1:0] r_tdata1;
    logic      [NUM_TRIGGERS-1:0][31:0] r_tdata2;

    assign o_trigger_mcontrol = r_tdata1;
    assign o_trigger_tdata2   = r_tdata2;

    // Helper logic: index into the selected trigger safely
    logic w_trigger_valid;
    assign w_trigger_valid = (r_tselect < NUM_TRIGGERS);

    assign o_debug_mode = r_debug_mode;
    assign o_debug_target = DEBUG_HALT_ADDR;
    // ebreak inside Debug Mode triggers a jump to DEBUG_HALT_ADDR, not an exception
    logic w_is_debug_ebreak;
    assign w_is_debug_ebreak = r_debug_mode && i_exc_valid && (i_exc_cause == EXC_BREAKPOINT);
    
    assign o_debug_exc_taken = r_debug_mode && i_exc_valid && !w_is_debug_ebreak;
    assign o_debug_exc_target = DEBUG_EXCEPTION_ADDR;
    ...
    // -----------------------------------------------------------------------
    logic r_trap_just_taken;
    logic r_step_active;
    assign o_step_stall = r_step_active && i_id_ex_valid; // Stall IF, Flush ID after instruction enters EX

    logic w_debug_entry;
    logic [2:0] w_debug_cause;

    assign o_debug_taken = w_debug_entry;
    ...
    always_comb begin
        ...
        if (!r_debug_mode) begin
            ...
            end else if (i_trigger_match) begin
                w_debug_entry = 1'b1;
                w_debug_cause = 3'd2;  // trigger
            end else if (i_exc_valid && i_exc_cause == EXC_BREAKPOINT &&
                         r_dcsr[15]) begin  // ebreakm
                w_debug_entry = 1'b1;
                w_debug_cause = 3'd1;  // ebreak
            end else if (r_dcsr[2] && r_step_active && (i_instr_retired || r_trap_just_taken)) begin  // step
                w_debug_entry = 1'b1;
                w_debug_cause = 3'd4;  // step
            end
        end else begin
            if (w_is_debug_ebreak) begin
                w_debug_entry = 1'b1;
            end
        end
    end
    ...
    always_comb begin
        ...
        unique case (i_csr_addr)
            ...
            // Trigger CSRs
            CSR_TSELECT:    w_csr_rdata = r_tselect;
            CSR_TDATA1:     w_csr_rdata = w_trigger_valid ? 32'(r_tdata1[r_tselect]) : 32'd0;
            CSR_TDATA2:     w_csr_rdata = w_trigger_valid ? r_tdata2[r_tselect] : 32'd0;
            CSR_TINFO:      w_csr_rdata = w_trigger_valid ? 32'h0000_0004 : 32'd1; // Bit 2 set = type 2 (mcontrol) supported. Default is 1 (info=1)
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            ...
            for (int i = 0; i < NUM_TRIGGERS; i++) begin
                r_tdata1[i]         <= '0;
                r_tdata1[i].type_id <= TRIG_TYPE_MCONTROL;  // Triggers must advertise type on reset
                r_tdata2[i]         <= 32'd0;
            end
            ...
            // ---- Single-Step State Tracker ----
            r_trap_just_taken <= o_trap_taken;
            if (o_dret_taken && r_dcsr[2]) begin
                r_step_active <= 1'b1;
            end else if (w_debug_entry) begin
                r_step_active <= 1'b0;
            end
            ...
            if (w_debug_entry) begin
                if (!r_debug_mode) begin
                    r_debug_mode   <= 1'b1;
                    r_dpc          <= w_is_debug_ebreak ? i_exc_pc : i_async_epc;
                    r_dcsr[8:6]    <= w_debug_cause;
                    r_dcsr[1:0]    <= r_priv;  // Save current privilege
                end
                // If entering debug mode FROM debug mode (e.g. ebreak), dpc/dcsr/priv are NOT updated!
            ...
            // ---- Trap Entry ----
                ...
                // Save state
                r_mepc         <= i_exc_valid ? i_exc_pc : i_async_epc;
            ...
            // ---- CSR writes ----
                ...
                unique case (i_csr_addr)
                    ...
                    // Trigger CSRs
                    CSR_TSELECT: r_tselect <= w_csr_wval;
                    CSR_TDATA1: begin
                        if (w_trigger_valid) begin
                            // Hardware trigger data 1 (mcontrol)
                            mcontrol_t new_tdata1;
                            new_tdata1 = mcontrol_t'(w_csr_wval);
                            
                            // Only allow type=2 (mcontrol) writes. If they try to write another type, ignore or clamp to 2
                            // Also ensure dmode allows write access depending on current mode
                            // Simplified for K10: Just accept the write but explicitly force type_id = 4'h2
                            new_tdata1.type_id = TRIG_TYPE_MCONTROL;
                            // Reset unsupported fields
                            new_tdata1.maskmax = 6'd0;   // No mask support
                            new_tdata1.reserved = 1'b0;
                            new_tdata1.reserved0 = 1'b0;
                            
                            r_tdata1[r_tselect] <= new_tdata1;
                        end
                    end
                    CSR_TDATA2: begin
                        if (w_trigger_valid) begin
                            r_tdata2[r_tselect] <= w_csr_wval;
                        end
                    end
                    CSR_TINFO: ; // Read-only
