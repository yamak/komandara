...
```
komandara/
├── rtl/k10/                         # K10 CPU core RTL
│   ├── komandara_k10_pkg.sv         # Central package (opcodes, structs, types)
│   ├── k10_fetch.sv                 # IF stage — PC, alignment buffer
│   ├── k10_decode.sv                # ID stage — instruction decode, control
│   ├── k10_compressed_decoder.sv    # RV32C → RV32I expansion
│   ├── k10_execute.sv               # EX stage — ALU, branch resolution, forwarding
│   ├── k10_alu.sv                   # Arithmetic Logic Unit
│   ├── k10_mul_div.sv               # M extension — multiply/divide unit
│   ├── k10_imm_gen.sv               # Immediate generator
│   ├── k10_memory.sv                # MEM stage — pipeline wrapper for LSU
│   ├── k10_lsu.sv                   # Load/Store Unit (aligned + unaligned + AMO)
│   ├── k10_writeback.sv             # WB stage — result selection
│   ├── k10_regfile.sv               # 32-entry register file
│   ├── k10_hazard_unit.sv           # Forwarding, stalls, flushes
│   ├── k10_csr.sv                   # M/U mode CSRs, trap handling
│   ├── k10_pmp.sv                   # Physical Memory Protection
│   ├── k10_tracer.sv                # Instruction tracer (sim only, RISC-DV compatible)
...
│       ├── tb_mul_div.sv            # Standalone mul/div testbench
│       ├── tb_mul_div.cpp           # Standalone mul/div C++ driver
│       ├── komandara.vlt            # Verilator lint waivers
│       ├── riscv_core_setting.sv    # RISC-DV core configuration
│       ├── testlist.yaml            # RISC-DV test list
│       └── k10_link.ld              # Linker script
├── ip/                              # Reusable IP modules (generation-agnostic)
│   ├── common/                      # Shared primitives
│   │   └── rtl/
│   │       ├── komandara_skid_buffer.sv
│   │       ├── komandara_arbiter.sv
│   │       ├── komandara_bram.sv
│   │       ├── komandara_bus2axi4lite.sv
│   │       └── komandara_bram_axi4lite.sv
│   ├── axi4lite/                    # AXI4-Lite bus IP
│   │   └── rtl/
│   │       ├── komandara_axi4lite_pkg.sv
│   │       ├── komandara_axi4lite_master.sv
│   │       ├── komandara_axi4lite_slave.sv
│   │       └── komandara_axi4lite_xbar.sv
│   └── axi4/                        # AXI4 Full bus IP
│       └── rtl/
│           ├── komandara_axi4_pkg.sv
│           ├── komandara_axi4_master.sv
│           ├── komandara_axi4_slave.sv
│           └── komandara_axi4_xbar.sv
├── sw/k10/                          # Software tests
│   ├── smoke_test.S                 # Basic arithmetic smoke test
│   └── unaligned_test.S             # Self-checking unaligned memory access test
├── scripts/                         # Build & verification scripts
│   ├── setup_tools.sh               # One-time tool installation
│   ├── env.sh                       # Environment setup (portable)
│   ├── run_riscv_dv.sh              # RISC-DV verification flow (with Spike comparison)
│   ├── run_selfcheck_test.sh        # Self-checking test runner (no Spike)
│   └── verilog_byte2word.py         # Hex file converter
